...
1. Create a new Render Web Service and point it at this repo.
2. Render will detect the included `render.yaml` and Dockerfile.
3. Once deployed, open the service URL Render provides to access the UI.

### Docker (self-host)

```bash
docker build -t research-pdf-agent .
docker run -p 8000:8000 research-pdf-agent
```
